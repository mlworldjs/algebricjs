import { cAdd } from './compute-engines/compute-engine-factory';
...
/**
 * Adds two matrices of the same shape and returns the result.
 * Only supports 2D matrices and throws an error if the matrices have different shapes
 * or if the inputs are not 2D matrices. Both matrices must be arrays of arrays of numbers.
 *
 * @param m1 - The first matrix to add. It must be a 2D array where each sub-array represents a row of the matrix.
 * @param m2 - The second matrix to add. It must have the same shape as `m1`.
 * @returns A new matrix that is the element-wise sum of `m1` and `m2`.
 ...
 *
 * @example
 * const result = matAdd([[1, 2], [3, 4]], [[5, 6], [7, 8]]);
 * console.log(result); // Outputs: [[6, 8], [10, 12]]
 */
export function matAdd(m1: Matrix, m2: Matrix) {
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      results[i][j] = cAdd(m1[i][j], m2[i][j]);
    }
  }
